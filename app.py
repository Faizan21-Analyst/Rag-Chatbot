...
# ── Backend import with visible error if it fails ─────────────
try:
...
def load_conversation(thread_id):
    state = rag_work.get_state(config={"configurable": {"thread_id": thread_id}})
...
def get_ai_response(user_input: str, thread_id: str) -> str:
    ...
    result = rag_work.invoke(
        {"messages": [HumanMessage(content=user_input)], "thread_id": thread_id},
        config=CONFIG,
    )
...
with st.sidebar:
    ...
    ingested = [
        fname for (tid, fname) in st.session_state["uploaded_files_info"]
...
if user_input:
    ...
    st.session_state["message_history"].append({"role": "AI", "content": ai_reply})
